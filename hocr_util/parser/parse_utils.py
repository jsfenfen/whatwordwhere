...
def get_words_only(word_tree):
    ...
    for word in hocr_words:
        ...
        word_array.append({'bbox':get_bbox_from_title(word.attrib['title']), 'text':tostring(word, method="text", encoding='UTF-8')})
    return word_array
    

def get_words_from_line(etree, line_id):
    word_array = []
...
def get_lines_with_words(line_tree):
    ...
    for line in hocr_lines:
        ...
        this_line['words'] = get_words_from_line(line, line.attrib['id'])
        line_array.append(this_line)
    return line_array
...
def get_words_with_lines_from_page(hocr_page):
    ...
    lines = get_lines_with_words(tree)
    return {'attrib': page_details, 'lines':lines}
